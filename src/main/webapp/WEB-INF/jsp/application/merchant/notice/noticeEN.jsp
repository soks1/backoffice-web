				...
				<div>
					...
					<div class="widget-body">
						<form class="form-horizontal" id="frmNoti" >
							...
							<input type="hidden" name="del_file_yn"  value="N" />
							...
							<div class="form-actions">
								<div class="row">
									<div class="col-md-12">
										<a href="javascript:f_updateNotice();" class="btn btn-primary btn_noti_create">공지사항 수정</a>
										<a href="javascript:f_noticeInfo();" class="btn btn-default">취소</a>
									</div>
								</div>
							</div>
						</form>
...
var pagefunction = function() {
	...
	$.ajax({
	    ...
	    success: function(obj){
	    	...
	    	$('#noti_divn_cd').val(noticeInfo.noti_divn_cd);
	    	$('#frmNoti').find('input[name=title]').val(noticeInfo.title);
	    	...
	    	if(noticeInfo.atch_file_yn == 'Y') {
		    	$('#frmNoti').find('input[name=atch_file_nm]').val(noticeInfo.view_file_nm);
	    	...
	    	$('#frmNoti').find('input[name=mcht_nm_list]').val(noticeInfo.mcht_nm_list);
	    	$('#frmNoti').find('input[name=mcht_id_list]').val(noticeInfo.mcht_id_list);
	    	$('#frmNoti').find('textarea[name=content]').val(noticeInfo.content);
	    },
	    error: function(obj) {
...
function f_updateNotice() {
	...
	formData.append('noti_id'	  , $("#frmKeepData").find('input[name=noti_id]').val());
	formData.append('atch_file_yn', $('#frmNoti').find('input[name=atch_file_yn]').val());
	formData.append('del_file_yn' , $('#frmNoti').find('input[name=del_file_yn]').val());
	...
	if(!confirm("해당 내용으로 공지사항이 수정됩니다.")) {
		return;
	}
	
	$.ajax({
	    url: api_url+'/bo/notice/npmt_bo_02203',
	    type: 'POST',
	    data: formData,
	    async: true,
	    cache: false,
