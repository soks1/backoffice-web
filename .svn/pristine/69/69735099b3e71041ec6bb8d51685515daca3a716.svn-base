						...
						<div class="widget-body-toolbar" style="display: none;">
							<div class="row">
								<div class="col-sm-12 text-right">
									<a href="javascript:f_statCsvDownload('hour');" class="btn btn-default"><i class="fa fa-lg fa-download"></i>&nbsp;Download</a>
								</div>
							</div>
						</div>
...
function f_resetCalendar(){
	$('input[name=start_date]').val(cf_termDate(-1));
	$('input[name=end_date]').val(cf_termDate(-1));
	
	$('input[name=start_date]').datepicker('option','maxDate', cf_termDate(-1));
	$('input[name=end_date]').datepicker('option','minDate', cf_termDate(-1));
}
...
function f_ajaxSubmit() {
	...
	// 실제 데이터 전송
	$.ajax({
		url :  api_url + '/bo/statistics/npmt_bo_07000',
	    type: 'GET',
	    async: true,
	    cache: true,
	    datatype: 'json',
...
function drawTable(jsonData) {
	...
	$.each(jsonData, function(j_i, j_item) {
		tag_td += '<th class="ta-c">' + j_item['stats_hour'] + '시</th>\n';
	});
...
function drawChart(dataset){
	...
	for(var nRow=0; nRow<dataset.length; nRow++){
		// Chart 데이터 처리 시작 =========================
		_label[nRow] = dataset[nRow].stats_hour + '시';
