...
<section id="widget-grid" class="">
					...
					<div class="widget-body">
						<form id="frmSearch" class="form-horizontal">
							<fieldset>
								...
								<div class="form-group">
									<label class="col-md-1 control-label">N-PMT주문ID</label>
									<div class="col-md-2">
										<input class="form-control" type="text" id="npmt_order_id" name="npmt_order_id">
									</div>
									<label class="col-md-2 control-label">조회일자</label>
									<div class="col-md-2">
										<div class="input-group">
											<input class="form-control datepicker pr12 tac" id="search_start_date" name="search_start_date" type="text" placeholder="시작일" data-dateformat='yy-mm-dd' data-valid-date='true' data-valid-use='true' data-valid-title="시작일">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
										</div>
									</div>
									<div class="col-md-2">
										<div class="input-group">
											<input class="form-control datepicker pr12 tac" id="search_end_date" name="search_end_date" type="text" placeholder="종료일" data-dateformat='yy-mm-dd' data-valid-date='true' data-valid-use='true' data-valid-title="종료일">
											<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
										</div>
									</div>
								</div>
	...
	<div class="form-actions mt0" id="csv_down">
		<div class="row">
			<div class="col-md-12">
				<a href="javascript:f_csvDownload();" class="btn btn-default"><i class="fa fa-lg fa-download"></i>&nbsp;Download</a>
			</div>
		</div>
	</div>
	<div class="row">
 ...
 * calendar init 
 * ----------------------------------------------------------------------------------
 * 종료일이 시작일을 넘길 수 없도록 셋팅
 * ==================================================================================
 */
function f_initDatePicker() {
	
	$('input[name=search_start_date]').datepicker(
	    $.extend({
	        onSelect: function(selected) {
	            $('input[name=search_end_date]').datepicker('option','minDate', selected);
	        }
	    }, date_opts)
	);
	$('input[name=search_end_date]').datepicker(
	    $.extend({
	        onSelect: function(selected) {
	            $('input[name=search_start_date]').datepicker('option','minDate', selected);
	        }
	    }, date_opts)
	);
...
function f_resetCalendar(){
	...
	$('input[name=search_start_date]').datepicker('option','maxDate', cf_termDate(-1));
	$('input[name=search_end_date]').datepicker('option','maxDate', cf_termDate(-1));
}
...
var pagefunction = function() {
	...
	// checkbox 데이터 추출
	var preqTypeList = new Array();
	
	var checkCnt = 0;
	$.each($('input[name=pmt_req_type_cd]'), function(idx, data) {
		if($(this).is(':checked')) {
			preqTypeList[checkCnt] = $(this).val();
			checkCnt++;
		}
	});
	
	if(preqTypeList.length == 0) {
		preqTypeList = null;
	}
		...
		jQuery("#tran_jqgrid").jqGrid({
			...
			onCellSelect: function(rowid, index, contents, event) {
				if(index == 0) {
					...
					var url = "/application/payment/all/paymentAllRN.do";
					var method = 'POST';
