							...
							<fieldset>
								<div class="form-group">
									...
									<div class="col-md-3half">
										<select class="form-control" id="s_stts_yy" name="s_stts_yy">
											<option value="2018">2018년</option>
										</select>
									</div>
									<label class="col-md-1 control-label ta-c" style="width:10px;"><strong>~</strong></label>
									<div class="col-md-3half">
										<select class="form-control" id="e_stts_yy" name="e_stts_yy">
											<option value="2018">2018년</option>
										</select>
									</div>
								</div>
						...
						<div class="widget-body-toolbar" style="display: none;">
							<div class="row">
								<div class="col-sm-12 text-right">
									<a href="javascript:f_statCsvDownload('month');" class="btn btn-default"><i class="fa fa-lg fa-download"></i>&nbsp;Download</a>
								</div>
							</div>
						</div>
...
function f_initYearSelect() {
	...
	$("#frmSearch select").change(function() {
		var start_select_val = $("#s_stts_yy option:selected").val();
		var end_select_val = $("#e_stts_yy option:selected").val();
		
		$("#s_stts_yy option").each(function(i,start) {
			if ($(start).val() > end_select_val) {
		...
		$("#e_stts_yy option").each(function(j, end) {
			if ($(end).val() < start_select_val) {
				console.log($(end).val());
...
function f_resetCalendar() {
	...
	$("#s_stts_yy option").each(function() {
	...
	$("#e_stts_yy option").each(function() {
		$(this).prop('disabled', false);
...
function f_ajaxSubmit() {
	...
	// 실제 데이터 전송
	$.ajax({
		url :  api_url + '/bo/statistics/npmt_bo_07004',
	    type: 'GET',
	    async: true,
	    cache: true,
	    datatype: 'json',
...
function drawTable(jsonData) {
	...
	$.each(jsonData, function(j_i, j_item) {
		tag_td += '<th class="ta-c">' + j_item['stats_mon'] + '월</th>\n';
	});
...
function drawChart(dataset){
	...
	for(var nRow=0; nRow<dataset.length; nRow++){
		// Chart 데이터 처리 시작 =========================
		_label[nRow] = dataset[nRow].stats_mon + '월';
